...
class ArcFaceEmbedderONNX:
    ...
    def __init__(
        ...
        from src.embed import input_layout_from_shape, preprocess_arcface_face
        self._input_layout = input_layout_from_shape(inp.shape)
        self._preprocess_arcface = preprocess_arcface_face
        if self.debug:
            print("[embed] model:", model_path)
            print("[embed] providers:", self.sess.get_providers())
            print("[embed] input:", inp.name, inp.shape, inp.type, "layout:", self._input_layout)
    ...
    def _preprocess(self, aligned_bgr_112: np.ndarray) -> np.ndarray:
        return self._preprocess_arcface(
            aligned_bgr_112,
            (self.in_w, self.in_h),
            self._input_layout,
        )
...
DEFAULT_STATUS_TOPIC = "vision/elvin/status"
DEFAULT_TARGET_NAME = "Elvin"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--directional-search-sec",
        type=float,
        default=3.0,
        help="When the target leaves the frame, turn toward the side they exited for this long before falling back to a full sweep. 0 disables directional search.",
    )
    parser.add_argument(
        "--exit-side-min-px",
        type=float,
        default=15.0,
        help="Minimum horizontal offset from center to record which side the target was last on (used for directional search).",
    )
    parser.add_argument(
        "--mqtt-min-interval",
        type=float,
...
def main():
    ...
    args.directional_search_sec = float(max(0.0, args.directional_search_sec))
    args.exit_side_min_px = float(max(0.0, args.exit_side_min_px))
    ...
    window_name = "Face Recognition - q or ESC to quit"
    cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
    cv2.resizeWindow(window_name, int(camera_width), int(camera_height))
    focus_cv_window(window_name)
    
    print(f"\nRecognize target speaker: {args.target_name} - Using {provider_name}")
    print("Controls: q/ESC=quit (click video window first), r=reload DB, +/- threshold, d=debug")
    ...
    try:
        while True:
            ...
            if force_command_now:
                # Reacquire / centering / stop decisions bypass the hold so the
                # servo reacts instantly the moment the target is recognized.
                visible_movement_command = movement_command
                visible_command_changed_at = current_time
            else:
            ...
            key_raw = pump_cv_key(25)
            ascii_key, arrow_key = decode_cv_key(key_raw)
            
            if arrow_key == 75 or ascii_key == ord('a'):  # Left arrow or 'a' - previous face
            ...
            elif arrow_key == 77 or ascii_key == ord('f'):  # Right arrow or 'f' - next face
                if not face_lock and len(faces) > 0:
                    if selected_face_index is None:
            ...
            if is_quit_key(ascii_key):
                print("[recognize] quit (q / ESC)")
                break
            elif ascii_key == ord("r"):  # Reload DB
            ...
            elif ascii_key in (ord("+"), ord("=")):  # Increase threshold
            ...
            elif ascii_key == ord("-"):  # Decrease threshold
                matcher.dist_thresh = float(max(0.05, matcher.dist_thresh - 0.01))
                print(f"[recognize] thr(dist)={matcher.dist_thresh:.2f} (sim~{1.0-matcher.dist_thresh:.2f})")
            elif ascii_key == ord("d"):  # Toggle debug
